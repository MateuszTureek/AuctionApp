...
namespace AuctionApp.Core.DAL.Specyfication
{
    public class OrderBoughtItemSpecyfication : ISpec<Item, object>
    {
        ...
        public OrderBoughtItemSpecyfication(BoughtItemsOrderBy orderBy)
        {
            ...
            switch (_orderBy)
            {
                case BoughtItemsOrderBy.Payment: { _expression = (x => x.Payment.Name); break; }
                case BoughtItemsOrderBy.BuyNowPrice: { _expression = (x => x.ConstPrice); break; }
                case BoughtItemsOrderBy.EndDate: { _expression = (x => x.AuctionEnd); break; }
                case BoughtItemsOrderBy.StartDate: { _expression = (x => x.AuctionStart); break; }
